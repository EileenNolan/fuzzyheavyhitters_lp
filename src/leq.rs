    ...
    C: AbstractChannel + Clone,
{
    ...
    let eq = fancy_leq_zero(&mut gb, wires, num_tests).unwrap(); //e addition
...
pub trait BinaryGadgets: FancyBinary + BundleGadgets {
    ...
    fn bin_leq_zero(
        ...
        let bits = x.wires();
        let msb = &bits[0]; // Assuming MSB is at the beginning
        let is_zero = {
            let mut not_bits = Vec::with_capacity(bits.len());
            for bit in bits {
                let not_bit = self.negate(bit)?;
                not_bits.push(not_bit);
            }
            self.and_many(&not_bits)?
        };
        self.or(msb, &is_zero)
    }
}
...
{
    let x = &wire_inputs.evaluator_wires;
    let string_len = x.wires().len() / num_tests;

    let mut results = Vec::with_capacity(num_tests);

    for i in 0..num_tests {
        let slice = &x.wires()[i * string_len..(i + 1) * string_len];
        let result = f.bin_leq_zero(&BinaryBundle::new(slice.to_vec()))?;
        results.push(result);
    }
...
fn eq_gc() {
    ...
    let expected = ev_value
    .iter()
    .map(|val| {
        let signed: i16 = i16::from_str_radix(
            &val.iter().map(|b| b.to_string()).collect::<String>(),
            2,
        ).unwrap();
        signed <= 0
    })
    .collect::<Vec<bool>>(); //e addition
